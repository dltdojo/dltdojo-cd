...
async function register(username) {
    ...
    makeCredResponse = {
        ...
        rawId: encodeBase64Url(navCredCreateResp.rawId),
        transports: navCredCreateResp.response.getTransports ? navCredCreateResp.response.getTransports() : undefined,
        response: {
            attestationObject: encodeBase64Url(navCredCreateResp.response.attestationObject),
            clientDataJSON: encodeBase64Url(navCredCreateResp.response.clientDataJSON)
        },
...
function preformatMakeCredReq(makeCredReq){
	makeCredReq.challenge = decodeBase64Url(makeCredReq.challenge).buffer;
	makeCredReq.user.id = decodeBase64Url(makeCredReq.user.id).buffer;
	// Decode id of each excludeCredentials
	if (makeCredReq.excludeCredentials) {
		makeCredReq.excludeCredentials = makeCredReq.excludeCredentials.map((e) => { return { id: decodeBase64Url(e.id).buffer, type: e.type };});
	}
	return makeCredReq;
};
function preformatGetAssertReq(getAssert) {
	getAssert.challenge = decodeBase64Url(getAssert.challenge).buffer;
	// Allow any credential, this will be handled later
	for(let allowCred of getAssert.allowCredentials) {
		allowCred.id = decodeBase64Url(allowCred.id).buffer;
	}
	return getAssert;
};
const publicKeyCredentialToJSON = (pubKeyCred) => {
	...
	if(pubKeyCred instanceof ArrayBuffer) {
		return encodeBase64Url(pubKeyCred);
	}
