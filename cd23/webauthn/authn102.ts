...
class Fido2Utils {
    ...
    static getAllowCredentialsFromUserAuthenticators = (authenticators: Authenticator[]) => {
...
class UserUtils {
    ...
    static upsertUser = async (user: User) => {
...
class SessMgr {
    ...
    setPayload = async <T,>(payload: T, key: string[], sessionTtlSeconds: number) => {
    ...
    getSessionFromRequestCookie = async <T,>(req: Request, cookieName: string, keyPrefix: string) => {
        const sessionId = getCookies(req.headers)[cookieName];
        const sessionPayload: T | null = await this.getPayload<T>([keyPrefix, sessionId]);
        return { sessionId, sessionPayload };
    }

    setSessionCookie = (response: Response, cookieName: string, sessionId: string, isCookieSecure: boolean, sessionTtlSeconds: number) => {
    ...
    deleteSessionAndCookie = async (response: Response, cookieName: string, key: string[]) => {
...
class CookieUtils {
    ...
    static isSecure = (requestUrl: string) => new URL(requestUrl).protocol === "https:";

    // Dynamically prefixes the cookie name, depending on whether it's for a secure origin (HTTPS).
    static getCookieName = (name: string, isSecure: boolean) => isSecure ? "__Host-" + name : name;
...
class SessionUtils {
    static newChallengeSession = async (sessId: string, challenge: string, username: string, req: Request, resp: Response) => {
        ...
        await sessMgr.setPayload(payload, sessDbKey, SOPT.CHALLENGE.sessionTtlSeconds)
        // 會期綁定
        const isCookieSecure = CookieUtils.isSecure(req.url)
        const cookieName = CookieUtils.getCookieName(SOPT.CHALLENGE.cookieName, isCookieSecure)
        sessMgr.setSessionCookie(resp, cookieName, sessId, isCookieSecure, SOPT.CHALLENGE.sessionTtlSeconds)
    }

    static newMemberSession = async (sessId: string, username: string, req: Request, resp: Response) => {
        const sessMgr = new SessMgr()
        // const sessId = SessMgr.RandomId()
        ...
        // 會期綁定
        const isCookieSecure = CookieUtils.isSecure(req.url)
        const cookieName = CookieUtils.getCookieName(SOPT.MEMBER.cookieName, isCookieSecure)
        sessMgr.setSessionCookie(resp, cookieName, sessId, isCookieSecure, SOPT.MEMBER.sessionTtlSeconds)
    }
}
...
class Handlers {
    static handleLogin = async (req: Request) => {
        ...
        if (userInfo === undefined) {
            ...
            authnOptions.allowCredentials = Fido2Utils.getAllowCredentialsFromUserAuthenticators(userInfo.authenticators);
            const response = HtmlUtils.respJson(authnOptions);
            // 新增登入挑戰會期
            const sessId = SessMgr.RandomId()
            await SessionUtils.newChallengeSession(sessId, authnOptions.challenge, username, req, response)
            return response;
        }
    }
    ...
    static handleRegister = async (req: Request) => {
        ...
        // 新增註冊會期
        await SessionUtils.newChallengeSession(sessId, registrationOptions.challenge, username, req, response)
        return response;
    };


    static handleRegisterAttestation = async (postBody: any, challenge: string, aUser: User): Promise<Response> => {
    ...
    static handleLoginAttestation = async (postBody: any, challenge: string, aUser: User) => {
        postBody.rawId = decodeBase64Url(postBody.rawId).buffer;
        const allowCredentials = Fido2Utils.getAllowCredentialsFromUserAuthenticators(aUser.authenticators);
    ...
    static handleAttestion = async (req: Request) => {
        ...
        const isCookieSecure = CookieUtils.isSecure(req.url)
        const cookieName = CookieUtils.getCookieName(SOPT.CHALLENGE.cookieName, isCookieSecure)
        const { sessionId, sessionPayload } = await sessMgr.getSessionFromRequestCookie<ChallengeSession>(req, cookieName, SOPT.CHALLENGE.sessionPrefix)
        ...
        if (username && challenge) {
            ...
            if (userInfo && postBody.response.attestationObject !== undefined) {
                ...
                resp = await Handlers.handleRegisterAttestation(postBody, challenge, userInfo)
                sessMgr.deleteSessionAndCookie(resp, cookieName, sessDbKey)
            } else if (userInfo && postBody.response.authenticatorData !== undefined) {
                // login attestation
                resp = await Handlers.handleLoginAttestation(postBody, challenge, userInfo)
                sessMgr.deleteSessionAndCookie(resp, cookieName, sessDbKey)
                if (resp.status == 200) {
                    await SessionUtils.newMemberSession(SessMgr.RandomId(), userInfo.userName, req, resp)
                }
            }
        }
        return resp;
