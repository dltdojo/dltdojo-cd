...
const COOKIE_SESSION_KEY = "scjwt";
...
async function pageAuth(req: Request): Response {
  const cookies = getCookies(req.headers);
  const payload = await verify(cookies[COOKIE_SESSION_KEY], JWTKEY); // { foo: "bar" }
  const sessionObj = sessionDb[payload.name101];
...
async function apiLogout(req: Request) {
  ...
  deleteCookie(resp.headers, COOKIE_SESSION_KEY, { path: "/" });
  return resp;
}
...
async function apiLogin(req: Request) {
  ...
  if (user === undefined) {
    ...
    const jwt101: string = await create({ alg: "HS256", typ: "JWT" }, { name101: user.username }, JWTKEY);
    sessionDb[user.username] = { loggedIn: true };
    const cookie: Cookie = {
