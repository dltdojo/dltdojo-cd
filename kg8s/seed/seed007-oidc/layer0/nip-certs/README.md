...
openssl req -out nip.io.csr -newkey ec:<(openssl ecparam -name secp256r1) -nodes -keyout nip.io.key -subj "/CN=*.127.0.0.1.nip.io/O=LOCALDEV"
openssl x509 -req -days 3650 -CA ca.crt -CAkey ca.key -set_serial 0x$(openssl rand -hex 8) \
  -extfile <(printf "subjectAltName=DNS:*.127.0.0.1.nip.io") \
  -in nip.io.csr -out nip.io.crt
```

result

```sh
$ openssl x509 -in nip.io.crt -text -noout
Certificate:
    Data:
        Version: 3 (0x2)
        Serial Number:
        ...
        Subject: CN = *.127.0.0.1.nip.io, O = LOCALDEV
